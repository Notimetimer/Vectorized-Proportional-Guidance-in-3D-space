...
vm0 = 360; % initial missile speed
vt0 = 360; % initial target speed
...
% q_beta=Q_beta(1)
% [hit,~]=MissileTest(dist0, heightt0, q_beta, q_epsilon, theta_t, vm0, vt0, shoot_method, target_move, count);

angle_list=[];
for q_epsilon=Q_epsilon
    for q_beta=Q_beta
        % 筛选掉南北极多余的经度
...
parfor i = 1:L % parfor
    ...
    while abs(a-b)>epsilon
        ...
        % 算λ点
        dist0=lambda;
        [hit2,count]=MissileTest(dist0, heightt0, q_beta, q_epsilon, theta_t, vm0, vt0, shoot_method, target_move, count);
        % 算b点
        if next_hit3==-1
            dist0=b;
            [hit3,count]=MissileTest(dist0, heightt0, q_beta, q_epsilon, theta_t, vm0, vt0, shoot_method, target_move, count);
        else
            hit3=next_hit3;
        end
        next_hit3=-1;
...
%         end
        ...
        if next_hit1==-1
            dist0=a;
            [hit1,count]=MissileTest(dist0, heightt0, q_beta, q_epsilon, theta_t, vm0, vt0, shoot_method, target_move, count);
        else
            hit1=next_hit1;
        end
        % 算λ点
